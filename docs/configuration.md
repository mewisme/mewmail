...
| `PUBLIC_URL` | — | Public API base URL for `preview_url` / `keep_url` in webhooks and `POST /emails/{id}/preview-token` |
...
When `PUBLIC_URL` is set, `email.received` payloads include one-click `preview_url` and `keep_url` links with separate tokens (`preview_otk` is one-time; `keep_otk` survives preview). Preview links are consumed on first view; use `POST /emails/{id}/preview-token` to issue a new preview token.
